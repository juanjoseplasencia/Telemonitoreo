...
GO
/****** Object:  StoredProcedure [dbo].[sproc_InsertRecordatorioReporteMonitoreo]    Script Date: 04/11/2016 12:16:08 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER  PROCEDURE [dbo].[sproc_InsertRecordatorioReporteMonitoreo]
AS
BEGIN
SET NOCOUNT ON
...
IF ((DatePart(hh,GETDATE()) = 12 AND DATEDIFF(mi, @FechaUltimaEjecucion, getdate()) >= 1260)  Or 
	...
	IF (DatePart(hh,GETDATE()) = 12)
		BEGIN
			...
			WHERE G.Eliminado  = 0 AND G.GestanteTelefono IS NOT NULL AND GM.GestanteKey IS NULL AND G.FechaProbableParto >= GETDATE()
