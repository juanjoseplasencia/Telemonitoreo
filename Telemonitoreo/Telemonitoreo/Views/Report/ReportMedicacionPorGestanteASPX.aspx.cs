...
{
    public partial class ReportMedicacionPorGestanteASPX : System.Web.Mvc.ViewPage<dynamic>
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (IsPostBack) return;
            var nroDocumento = ViewData["NroDocumentoGestante"].ToString();
            ...
            {
                List<sproc_ReportMedicacionGestante_Result> evolGestante;
                if (perfil == 2 && establecimiento > 0)
                {
                    evolGestante = dc.sproc_ReportMedicacionGestante().Where(g => g.GestanteNroDocumento == nroDocumento && g.EstablecimientoId == establecimiento &&
                                                                              (fechaIni == "" || g.Fecha>= fInicial) &&
                                                                              (fechaFin == "" || g.Fecha <= fFinal)).ToList();
                }
                else
                {
                    evolGestante = dc.sproc_ReportMedicacionGestante().Where(g => g.GestanteNroDocumento == nroDocumento &&
                                                                              (fechaIni == "" || g.Fecha >= fInicial) &&
                                                                              (fechaFin == "" || g.Fecha <= fFinal)).ToList();
                }
                
                ReportViewer1.ProcessingMode = ProcessingMode.Local;
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/RptMedicamentoGestante.rdlc");

                var rdc = new ReportDataSource("DSMedicacionGestante", evolGestante);
